    ...
    return (
        <div>
            <div style={{ marginBottom: '2rem' }}>
                <h1 style={{ fontFamily: 'Cinzel, serif', fontWeight: 700, fontSize: '1.6rem', color: '#111', marginBottom: '0.25rem' }}>Document Library</h1>
                <p style={{ fontFamily: 'Inter, sans-serif', fontSize: '0.85rem', color: '#9ca3af' }}>Upload and manage reports, brochures, press kits, and research documents</p>
            </div>
            ...
            >
                ...
                <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
                    <button className="btn-primary" style={{ fontSize: '0.82rem', padding: '10px 20px' }}>Browse Files</button>
                    <div style={{ display: 'flex', gap: 6 }}>
                        {['Research', 'Brochure', 'Report', 'Press Kit'].map((t) => (
                            <span key={t} className={`badge ${typeColors[t]}`}>+ {t}</span>
                        ))}
                    </div>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
                {[
                    { label: 'Total Documents', value: '5', color: '#1F6F3E' },
                    { label: 'Total Downloads', value: '689', color: '#C9A227' },
                    { label: 'Storage Used', value: '21.2 MB', color: '#B22222' },
                ].map((s) => (
                    <div key={s.label} className="stat-widget" style={{ borderLeftColor: s.color }}>
                        <div style={{ fontFamily: 'Cinzel, serif', fontWeight: 700, fontSize: '1.4rem', color: '#111' }}>{s.value}</div>
