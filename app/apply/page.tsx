...
function ApplyForm() {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        setPaymentError(
                            ...
                            ` Reference: ${response.reference} — please contact us if this persists.`
                        );
                        setPaymentLoading(false);
                    }
                } catch {
                    setPaymentError(`Network error during verification. Reference: ${response.reference} — please contact us.`);
                    setPaymentLoading(false);
                }
            },
    ...
    // Legacy direct-submit (used if payment is bypassed in dev)
                ...
                <div className="container-brand max-w-3xl relative z-10">
                    ...
                    <div className="card-brand animate-fade-up-delay-1 p-8 md:p-12 shadow-[var(--shadow-soft)]">
                        ...
                        {formData.assessmentScore !== undefined && (
                            <div style={{
                                background: 'rgba(31, 111, 62, 0.08)', border: '1.5px solid var(--color-primary)',
                        ...
                        <div className="flex justify-between items-center mb-10 relative">
                            ...
                            <div
                                className="absolute top-1/2 left-0 h-1 bg-[var(--color-primary)] -translate-y-1/2 z-0 transition-all duration-300"
                                style={{ width: fillWidth }}
                            />
                            {[1, 2, 3].map(n => (
                                <div key={n} className={`w-10 h-10 rounded-full flex items-center justify-center relative z-10 font-bold text-sm ${step >= n ? 'bg-[var(--color-primary)] text-white' : 'bg-white border-2 border-gray-200 text-gray-400'}`}>
                                    {n}
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="flex justify-between mb-8 -mt-6">
                            {['Personal Info', 'Program Details', 'Payment'].map((label, i) => (
                                <span key={label} className={`font-sans text-xs font-semibold uppercase tracking-wider ${step === i + 1 ? 'text-[var(--color-primary)]' : 'text-gray-400'}`} style={{ width: '33%', textAlign: i === 0 ? 'left' : i === 2 ? 'right' : 'center' }}>
                                    {label}
                                </span>
                            ))}
                        </div>

                        <form onSubmit={step === 2 ? (e) => { e.preventDefault(); handleNext(); } : (e) => { e.preventDefault(); handleNext(); }}>
                            {submitError && (
                        ...
                        {step === 3 && (
                            <div className="space-y-6 animate-fade-up">
                                ...
                                {/* Payment box */}
                                <div style={{
                                    background: 'linear-gradient(135deg, var(--color-primary) 0%, var(--color-primary-dark, #0d1f3c) 100%)',
                                    borderRadius: 10, padding: '1.5rem', color: '#fff',
                                }}>
                                    ...
                                    <p className="font-sans text-sm leading-relaxed" style={{ color: 'rgba(255,255,255,0.72)' }}>
                                        A one-time consultation fee is required to process your application. You will be connected with an adviser within 48 hours of payment.
                                    </p>
                                ...
                                {paymentError && (
                                    <div className="rounded-md border border-red-200 bg-red-50 px-4 py-3 font-sans text-sm font-medium text-red-700">
                                        {paymentError}
                                    </div>
                                )}

                                <div className="pt-2 flex flex-col md:flex-row gap-4 justify-between items-center">
                                    <button type="button" onClick={handlePrev} disabled={paymentLoading} className="font-sans text-sm font-semibold text-gray-500 uppercase tracking-wider hover:text-gray-900 transition-colors w-full md:w-auto text-center disabled:opacity-40">
                                        ← Back
                                    </button>
                                    ...
                                    {paystackPublicKey ? (
                                        ...
                                        >
                                            {paymentLoading ? 'Processing Payment...' : 'Pay ₵1,500 & Submit →'}
                                        </button>
                                    ) : (
